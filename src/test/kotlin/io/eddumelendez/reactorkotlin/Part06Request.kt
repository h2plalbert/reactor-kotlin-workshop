...
class Part06Request {
    ...
    fun requestOneExpectSkylerThenRequestOneExpectJesse(flux: Flux<User>): StepVerifier {
        return flux.test(1).expectNext(User.SKYLER).thenRequest(1).expectNext(User.JESSE).thenCancel()
    }
    ...
    fun experimentWithLog() {
        ...
        flux.test(0).thenRequest(1).expectNextMatches({ true }).thenRequest(1).expectNextMatches({ true })
            .thenRequest(2).expectNextMatches({ true }).expectNextMatches({ true }).verifyComplete()
    }
    ...
    fun experimentWithDoOn() {
        ...
        flux.test().expectNextCount(4).verifyComplete()
    }

    // TODO Return a Flux with all users stored in the repository that prints "Starring:" on subscribe, "firstname lastname" for all values and "The end!" on complete
    fun fluxWithDoOnPrintln(): Flux<User> {
        return repository.findAll().doOnSubscribe { println("Starring:") }
            .doOnNext { println("${it.firstname} ${it.lastname}") }.doOnComplete { println("The end!") }
    }
