...
class Part08OtherOperations {
    ...
    fun zipFirstNameAndLastName() {
        ...
        userFlux.test().expectNext(User.SKYLER, User.JESSE, User.WALTER, User.SAUL).verifyComplete()
    }

    // TODO Create a Flux of user from Flux of username, firstname and lastname.
    fun userFluxFromStringFlux(
        usernameFlux: Flux<String>, firstnameFlux: Flux<String?>, lastnameFlux: Flux<String?>
    ): Flux<User> {
    ...
    fun fastestMono() {
        ...
        mono.test().expectNext(MARIE).expectComplete().verify()
    }
    ...
    fun fastestFlux() {
        ...
        flux.test().expectNext(MARIE, MIKE).verifyComplete()
        ...
        flux.test().expectNext(User.SKYLER, User.JESSE, User.WALTER, User.SAUL).verifyComplete()
    }
    ...
    fun complete() {
        ...
        completion.test().verifyComplete()
    }
    ...
    fun nullHandling() {
        ...
        mono.test().expectNext(User.SKYLER).verifyComplete()

        mono = nullAwareUserToMono(null)
        mono.test().verifyComplete()
    }
    ...
    fun nullAwareUserToMono(user: User?): Mono<User> {
        return user?.toMono() ?: Mono.empty()
    }

    @Test
    fun emptyHandling() {
        var mono = emptyToSkyler(User.WALTER.toMono())
        mono.test().expectNext(User.WALTER).verifyComplete()
