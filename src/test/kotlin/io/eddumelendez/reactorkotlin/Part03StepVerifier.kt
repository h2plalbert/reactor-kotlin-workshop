...
//        StepVerifier.create(flux).expectNext("foo", "bar").verifyComplete()
        flux.test().expectNext("foo", "bar").verifyComplete()
    }
    ...
    fun expectSkylerJesseComplete(flux: Flux<User>) {
        StepVerifier.create(flux).expectNextMatches { it.username == "swhite" }.expectNextMatches { it.username == "jpinkman"}.verifyComplete()
    }
